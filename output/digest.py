...
output/digest.py
-----------------
Daily email digest for the Code Analysis & Review.
...
<div style="max-width:640px;margin:0 auto;padding:24px">
  <div style="background:#0d1117;border-radius:12px 12px 0 0;padding:20px 24px">
    <div style="color:#fff;font-size:20px;font-weight:700">📊 Code Analysis & Review Daily Digest</div>
  ...
  <div style="background:#fff;padding:22px 24px;border-radius:0 0 12px 12px;border:1px solid #e8eaed;border-top:none">
    ...
    <div style="margin-top:18px;font-size:11px;color:#9fadbf;text-align:center">
      Generated by Code Analysis & Review · This is an automated digest.
    </div>
  </div>
</div>
</body></html>"""
...
def render_digest_text(d: dict) -> str:
    ...
    lines = [
        f"Code Analysis & Review Daily Digest — {d['generated_at']} (last {d['window_days']} day(s))",
        "",
...
def send_digest(settings=None, days: int = 1) -> dict:
    ...
    msg["Subject"] = (f"Code Analysis & Review Digest — {len(data['blocks'])} blocked, "

...
class MaintainabilityAgent(BaseAgent[MaintainabilityResult]):
    ...
    system_prompt = (
        "You are a maintainability and clean-code expert for enterprise banking systems, reviewing at "
        ...
        "  - Duplication (DRY): logic copy-pasted or re-implemented instead of reused, especially across "
        "the files touched in THIS diff\n"
        "  - Naming clarity: a name that misleads about what the value/function actually does or hides "
        "a unit/type (e.g. a variable called `timeout` that's actually milliseconds when the codebase "
        "convention is seconds)\n"
        "  - Consistency: this change doing something a different way than the established pattern "
        "elsewhere in the same diff (different error-handling style, different naming convention) with "
        "no stated reason\n"
        "  - Documentation/comment debt: a public API or non-obvious business rule changed with no "
        "explanation of WHY, where a future reader would have no way to know\n"
        "  - Coupling/testability: a hard-coded dependency (new date/random/network call, singleton "
        "reach-through) that makes the changed code harder to unit test\n\n"
        "For each issue provide: kind, severity (low|medium|high), description, "
        "file_path, line number, and a concrete suggestion — the suggestion must be actionable "
        "(what to change it to), not just \"consider refactoring\". "
